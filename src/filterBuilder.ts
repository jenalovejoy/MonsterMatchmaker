$(document).ready(async(): Promise<any> => {
    ...
    const monsterTypes: String[] = ["Aberration", "Beast", "Celestial", "Construct", "Dragon", "Elemental", "Fey", "Fiend", "Giant", "Humanoid", "Monstrosity", "Ooze", "Plant", "Undead"];
    
    const alignmentA: String[] = ["Lawful", "Neutral", "Chaotic"];
    const alignmentB: String[] = ["Good", "Neutral", "Evil"];

    const alignment: String[] = buildMultidimentionalOptions(alignmentA, alignmentB);
...
function buildLabelTable(container: String, elements: String[], title: String, width: number): void {

    const numItems: number = elements.length;
    const numRows: number = numItems / width;
...
function buildMultidimentionalOptions(rows: String[], columns: String[]): String[] {
